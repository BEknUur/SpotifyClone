...
const AddToPlaylistModal = ({closeModal , addSongToPlaylist}) =>{
    ...
    return (
        <div className="text-white absolute w-screen h-screen bg-black bg-opacity-50 flex justify-center items-center" onClick={closeModal}>
            <div 
                className="bg-app-black w-1/3 rounded-md p-4 overflow-auto max-h-96"  
                onClick={(e) => {
                    e.stopPropagation();
                }}
            >
...
const PlaylistListComponent = ({info , addSongToPlaylist}) =>{
    return(
        <div className='bg-app-black flex w-full items-center space-x-4 hover:bg-gray-400 hover:bg-opacity-20 cursor-pointer p-3' 
